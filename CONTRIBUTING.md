...
| Type | Guidelines |
| :--- | :--- |
| **🐛 Bug Reports** | Use the issue template. Include minimal reproduction steps, expected vs actual behavior, Laravel/PHP/Spatie versions. |
| **✨ Feature Requests** | Describe the use case. Must fit the package scope (pivot activity logging, activity queries). |
| **🔧 Fluent APIs** | For `src/Support/`. Follow fluent/chainable conventions. Maintain Spatie Activity Log compatibility. |
| **🧩 Traits** | For `src/Traits/`. Keep traits focused and single-purpose. Ensure Eloquent compatibility. |
